...
def decode(name, sectorid):
    alphabet = string.ascii_lowercase
    mapping = {char: alphabet[(idx + sectorid) % 26]
...
def main():
    ...
    total = 0
    northpole_sectorid = None
    for line in lines:
        name, sectorid, checksum = parse(line)
        if is_valid(name, checksum):
            total += sectorid
            cleartext_name = decode(name, sectorid)
            if cleartext_name == 'northpole object storage':
                northpole_sectorid = sectorid

    print(total)
    print(northpole_sectorid)
